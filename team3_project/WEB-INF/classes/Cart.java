...
/* displayCart Function shows the products that users has bought, these products will be displayed with Total Amount.*/
	...
	protected void displayCart(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(product_id!=null){
			ArrayList<OrderItem> orderItems = OrdersHashMap.orders.get(utility.username());
			int index = 0;

			for (OrderItem oi : orderItems){
				if(oi.getName().equals(product_id)){
					break;
				}
				else
					index++;
		...
		if(utility.CartCount()>0)
		{
			...
			{				
				pw.print("<tr>");
				pw.print("<td style='font-size:14px; text-align:center;'>"+i+".</td>");
				pw.print("<td style='font-size:14px;'>"+oi.getName()+"</td>");
				pw.print("<td style='font-size:14px;'>$"+oi.getPrice()+"</td>");
				//pw.print("<td style='text-align:center'><input type='text' name='value' style='width:20px; height:10px;'></td>");
				pw.print("<td style='font-size: 14px; text-align:center;'><a href=\"Cart?product_id="+oi.getName()+"\">Cancel</a></td>");
			...
			pw.print("<tr style='font-size:14px; text-align:center;'><th></th><th>Total</th><th>"+total+"</th>");
